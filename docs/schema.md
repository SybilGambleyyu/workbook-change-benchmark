...
WCAB 0.41 adds one legacy shared-workbook revision log whose synthetic historic
value changes while the relationship-backed workbook/header/log graph, content
types, controls, ordinary cells, formulas, and all other package members remain
fixed. It records stored audit-trail material only; it does not expose historic
record material or prove provenance, identity, conflict resolution, review,
approval, authorization, or workflow behavior.
Version 2 remains
...
| `shared_workbook_revision_log_changed` | `workbook_relationships_member`, `revision_headers_member`, `revision_headers_relationships_member`, `revision_log_member`, `revision_headers_content_type`, `revision_log_content_type`, `revision_headers_relationship_type`, `revision_log_relationship_type`, `revision_header_part_count`, `revision_header_count`, `revision_log_part_count`, `revision_log_entry_count`, `shared_workbook_enabled_count`, `track_revisions_enabled_count`, `revision_history_enabled_count`, `keep_change_history_enabled_count`, `revision_history_protected_count`, `unrecognized_shared_workbook_revision_count`, `stable_sheet`, `stable_value_cell`, `stable_value`, `stable_formula_cell`, `stable_formula` | One relationship-backed legacy shared-workbook revision history retains its header/log graph, content types, tracking/retention controls, record shape, ordinary cells/formula context, calculation properties, and every package member except its revision-log part while one synthetic historic value changes. The validator reads bounded local OOXML only: it does not expose historic values, locations, author identity, timestamps, GUIDs, or relationship IDs, prove provenance or conflict resolution, replay history, or claim review/approval, authorization, or workflow behavior. |
...
describes revision history persisted as headers and log files, including stored
shared/tracking/history/retention/protection controls. The Open XML
...
then exposes its revision-log parts. That makes a relationship-backed revision
log a review surface even where today's ordinary cells and formulas agree.

WCAB 0.41 contains one `xl/revisions/revisionHeaders.xml` part, one linked
`xl/revisions/revisionLog1.xml` part, and one fixed header-to-log relationship.
Its header controls, synthetic author/timestamp/GUID, record structure,
content types, `Controls!B10=12`, `Controls!D10=B10*C10`, calculation
properties, and every other package member stay fixed; only one synthetic
historic value inside the revision log changes. The raw validator checks that
bounded local graph and compares the log after erasing that private value.

The pair records stored audit-trail material only. It does not publish historic
values, locations, writer identity, timestamps, GUIDs, or relationship IDs;
verify provenance or a writer; replay revisions; resolve a conflict; prove a
review/approval; authenticate or authorize a person; or claim workflow or
client behavior.
